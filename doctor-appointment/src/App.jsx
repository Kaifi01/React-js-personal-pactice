...
//post
 const addedtata=async()=>{
  const newpation={
      name: name,
      email: email
    };
    const res= await axios.post("http://localhost:3001/Pation", newpation);
...
getalldoctor()
  ...
  return (
   <>
   {doctors.map((item)=>   <Card key={item.id} item={item}/>)}
     <main>
  <div className="flex items-center h-screen px-2 sm:px-0">
    <form className="bg-gray-100 w-full max-w-sm sm:max-w-xl mx-auto p-4 rounded-xl shadow-md">
